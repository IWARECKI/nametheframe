...
function playTicketRevealSound() {
  // Paper tear / rip sound — short noise burst with high-pass character
  try {
    ...
    for (let i = 0; i < bufferSize; i++) {
      // Shaped noise — louder start, quick fade, crinkle character
      const env = Math.exp(-i / (bufferSize * 0.15));
      data[i] = (Math.random() * 2 - 1) * env * 0.4;
    }
    const noise = ctx.createBufferSource();
    noise.buffer = buffer;
    // High-pass filter for paper character
